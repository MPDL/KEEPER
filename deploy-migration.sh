...
# even inside set +e or subshells, as seen in the crash where "sourcing..." printed then rc=1 trap).
...
# only kills the child bash, never reaches our main shell or trap with rc=1).
...
if $is_background || [ "$NODE_TYPE" = "SINGLE" ]; then
    ...
    if [ -f "$SQLF" ] && command -v mysql >/dev/null 2>&1; then
        ...
        # Safe parse from the ini for __DB_* etc (never source inject here, to avoid exit 1 killing the deploy)
...
if ! $is_background || [ "$NODE_TYPE" = "SINGLE" ]; then
    ...
    if [ -d "$KEEPER_MIGRATION_SRC" ]; then
        ...
        if [ -d "$KEEPER_MIGRATION_DEST" ]; then
            ...
            if [ ! -d "$BACKUP" ]; then
                echo "Backing up existing $KEEPER_MIGRATION_DEST -> $BACKUP"
                mv "$KEEPER_MIGRATION_DEST" "$BACKUP" || true
            fi
        fi
        cp -a "$KEEPER_MIGRATION_SRC" "$KEEPER_MIGRATION_DEST"
        echo "    keeper/migration Python package deployed."
    fi
else
...
if [ "$MODE" = "standalone" ]; then
    ...
    if $is_background || [ "$NODE_TYPE" = "SINGLE" ]; then
        echo "  # .env (if not pre-placed; required for worker DB access and notifications):"
        echo "  cp .env.example .env ; edit .env   # (KEEPER_DB_HOST/USER/PASSWORD etc, optional email)"
