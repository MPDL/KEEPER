  ...
  var DirentView = HLItemView.extend({
    ...
    render: function() {
      ...
      if (this.model.get("is_dir")) {
        template =
          $(window).width() < 768 ? this.dirMobileTemplate : this.dirTemplate;
      } else {
        template =
          $(window).width() < 768 ? this.fileMobileTemplate : this.fileTemplate;
      }

      var can_set_folder_perm = false;
      if (
        app.pageOptions.folder_perm_enabled &&
        !dir.is_virtual &&
      ...
      this.$(".file-locked-icon").attr(
        "title",
        gettext("locked by {placeholder}").replace(
          "{placeholder}",
          this.model.get("lock_owner_name")
        )
      );
    ...
    itemDrop: function(e) {
      ...
      $.ajax({
        ...
        success: function() {
          var msg = gettext("Successfully moved %(name)s").replace(
            "%(name)s",
            obj_name
          );
    ...
    starFile: function() {
      ...
      if (starred) {
        $.ajax({
          url:
            Common.getUrl({ name: "starred_files" }) +
            "?repo_id=" +
            dir.repo_id +
            "&p=" +
    ...
    visitDirent: function() {
      if ($(window).width() < 768 && !this.model.get("is_img")) {
        // dir or non image file
    ...
    viewImageWithPopup: function() {
      var index = $(".img-name-link", this.dirView.$table).index(
        this.$(".img-name-link")
      );
    ...
    del: function() {
      ...
      this.model.deleteFromServer({
        success: function(data) {
          var msg = gettext("Successfully deleted %(name)s").replace(
            "%(name)s",
            dirent_name
          );
          Common.feedback(msg, "success");
    ...
    rename: function() {
      ...
      var after_op_success = function(data) {
        ...
        if (_this.model.get("is_img")) {
          var index = $(".img-name-link", _this.dirView.$table).index(
            _this.$(".img-name-link")
          );
          _this.dirView.updateMagnificPopupOptions({
            op: "update-item",
    ...
    viewDetails: function() {
      ...
      if (
        app.pageOptions.enable_thumbnail &&
        !this.dir.encrypted &&
        ...
        data.thumbnail_url = Common.getUrl({
          name: "thumbnail_get",
          repo_id: this.dir.repo_id,
          path: Common.encodePath(
            Common.pathJoin([this.dir.path, this.model.get("obj_name")])
          ),
          size: 1024
        });
      }
      ...
      if (this.model.get("perm") == "rw") {
        this.getTags = function() {
          $.ajax({
            ...
            cache: false,
            data: {
              path: Common.pathJoin([
                this.dir.path,
                this.model.get("obj_name")
              ]),
              is_dir: this.model.get("is_dir") ? true : false
            },
            dataType: "json",
