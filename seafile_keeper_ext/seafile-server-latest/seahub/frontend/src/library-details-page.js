          ...
          <div className="row">
            <div className="col-md-6 offset-md-1">
              ...
              {this.state.showDoi && (
                <div className="d-flex justify-content-between align-items-center op-bar">
                  <p className="m-0">
                    {gettext('Digital Object Identifier(DOI)')}
                  </p>
                </div>
              )}
              {this.state.showDoi && (
              ...
              <div className="table_row">
                <b>{gettext('Contact')}: </b>
                <a href={'mailto:' + owner_contact_email}>
                  {owner_contact_email}
                </a>
              </div>
            </div>
        ...
        return data.map((item, index) => (
          <tr key={index}>
            ...
            {item.path == '/' ? (
              <td>
                <a
                  href={`${siteRoot}bloxberg-cert/transaction/${item.transaction_id}/`}
                >
                  {gettext('Link to certificates')}
                </a>
              </td>
            ) : (
              <td>
                <a
                  href={`${siteRoot}bloxberg-cert/transaction/${item.transaction_id}/${item.checksum}/`}
                >
                  {gettext('Link to certificate')}
                </a>
              </td>
            )}
          </tr>
  ...
  render() {
    ...
    return (
      <table className="table-hover">
        ...
        <tbody>{this.renderTbody(data, type)}</tbody>
      </table>
    );
  }
}
