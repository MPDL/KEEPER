...
class RepoListItem extends React.Component {
    ...
    seafileAPI
      ...
      .then((res) => {
        ...
        if (this.props.isShowFile === true) {
          ...
          direntList = res.data.dirent_list.filter(
            (item) => item.type === "dir"
          );
        }
  ...
  loadNodeAndParentsByPath = (repoID, path, expandNode) => {
    ...
    seafileAPI
      .listDir(repoID, path, { with_parents: true })
      .then((res) => {
        ...
        for (let i = 0; i < direntList.length; i++) {
          ...
          let key =
            parentDir === "/" ? "/" : parentDir.slice(0, parentDir.length - 1);
          if (!results[key]) {
            results[key] = [];
          }
      ...
      <li>
        ...
        >
          <div className="item-text">
            <span
              className="name user-select-none ellipsis"
              title={this.props.repo.repo_name}
            >
          ...
          <div className="item-left-icon">
            <span
              className={`item-toggle icon fa ${
                this.state.isShowChildren ? "fa-caret-down" : "fa-caret-right"
              }`}
