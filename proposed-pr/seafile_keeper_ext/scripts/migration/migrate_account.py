...
The new worker (process_keeper_email_migrations.py) calls this script
as-is via subprocess, exactly like bulk_migrate.py did before.
...
def run_keeper_migration(migrate_server, ssh_user, from_api_email, to_api_email):
    ...
    print(f"\nRunning Keeper migration on {migrate_server}...")
    print(f"  Command: {cmd}")

    ssh = paramiko.SSHClient()
    ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    try:
        # Use the SSH agent (forwarded from local machine)
...
def _ssh_run(migrate_server, ssh_user, cmd):
    """Run a command on migrate_server over paramiko; return (exit_code, stdout, stderr)
    or (None, None, None) on connection failure (after printing the reason)."""
    ssh = paramiko.SSHClient()
    ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    try:
        ssh.connect(migrate_server, username=ssh_user, allow_agent=True)
...
def delete_sessions(migrate_server, ssh_user, from_api_email,
    ...
    print(f"\nDeleting old sessions for {from_api_email} on {migrate_server}...")
...
def main():
    ...
    if not migrate_server:
        print("Error: KEEPER_MIGRATE_SERVER is not set in .env.")
        sys.exit(1)
    if not ssh_user:
        print("Error: KEEPER_SSH_USER is not set in .env.")
        sys.exit(1)
    if not mysql_user:
